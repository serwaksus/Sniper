#!/usr/bin/env python3
"""
Health Checks — 25 individual check functions + helpers.
Grouped: System, Trading, Data checks.
"""
...
from schema import (
    EQUITY_SNAPSHOTS, EQUITY_TIMESTAMP, EQUITY_TOTAL, HEALTH_LAST_ALERTS, HEALTH_LAST_CYCLE_START, HEALTH_LAST_EQUITY,
)
...
def _check_error_spike(state: dict) -> tuple[str, str] | None:
    ...
    errors = sum(1 for line in hour_lines
                 if re.match(r'\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} (ERROR|CRITICAL)', line)
